		...
		function appendFormattedElement(&$wrapper, $data, $encode=false){
			...
			if ($this->get('allow_multiple') != 'yes') {
				...
				$hash = $this->Database->fetchVar('hash', 0, 
					"SELECT MD5(`value`) AS `hash` FROM `tbl_entries_data_".self::$_driver->memberEmailFieldID()."` WHERE `entry_id` = ".(int)$data['member_id']." LIMIT 1"
				);
			...
			foreach ($data['member_id'] as $index => $member_id) {
				...
				$hash = $this->Database->fetchVar('hash', 0, 
					"SELECT MD5(`value`) AS `hash` FROM `tbl_entries_data_".self::$_driver->memberEmailFieldID()."` WHERE `entry_id` = ".(int)$member_id." LIMIT 1"
				);
